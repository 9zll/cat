...
"""绘制单个类别变量饼图"""
def plot_pie_chart(data_table, variable_name):  
    # 提取变量的值  
    values = data_table[variable_name]  
  
    # 创建饼图  
    plt.pie(values, labels=variable_name, autopct='%1.1f%%')  
...
def 读取SPSS数据(文件所在位置及名称):   
     ...
     result, metadata = pyreadstat.read_sav(文件所在位置及名称, apply_value_formats=True, formats_as_ordered_category=True)    
